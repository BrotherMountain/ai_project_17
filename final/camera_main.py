    ...
    def run(self):
        while self.running and self.connect:
            ...
            if self.ret:  # 마스크 버튼 off일때
                ...
                # bbox, label, conf = cv.detect_common_objects(self.image, model='yolo4')
                #
                # print(bbox, label, conf)
                for idx, f in enumerate(face):

                    (self.startX, self.startY) = f[0], f[1]
                    (endX, endY) = f[2], f[3]

                    if 0 <= self.startX <= self.image.shape[1] and 0 <= endX <= self.image.shape[1] and 0 <= self.startY <= \
                            self.image.shape[
                                0] and 0 <= endY <= self.image.shape[0]:

                        face_region = self.image[self.startY:endY, self.startX:endX]
                        ...
                        if self.prediction < 0.7:  # 마스크 미착용으로 판별되면,
                            self.mask = False
                            cv2.rectangle(rgbImage, (self.startX, self.startY), (endX, endY), (0, 0, 255), 2)
                            Y = self.startY - 10 if self.startY - 10 > 10 else self.startY + 10
                            text = "No Mask ({:.2f}%)".format((1 - self.prediction[0][0]) * 100)
                            cv2.putText(rgbImage, text, (self.startX, Y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

                        else:  # 마스크 착용으로 판별되면
                            self.mask = True
                            cv2.rectangle(rgbImage, (self.startX, self.startY), (endX, endY), (0, 255, 0), 2)
                            Y = self.startY - 10 if self.startY - 10 > 10 else self.startY + 10
                            text = "Mask ({:.2f}%)".format(self.prediction[0][0] * 100)
                            cv2.putText(rgbImage, text, (self.startX, Y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
